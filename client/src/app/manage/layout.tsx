...
import {
  LayoutDashboard, FolderKanban, Users, LogOut, ChevronLeft, ChevronRight, Menu, X
} from "lucide-react"
...
export default function ManageLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = () => (
    <>
      ...
      {/* Bottom */}
      <div className="px-2 py-4 space-y-1" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        <button onClick={logout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors" style={{ color: "var(--t3)" }}>
    ...
    <div className="flex min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid var(--border)", background: "var(--surface)" }}>
          <button onClick={() => setMobileOpen(true)} style={{ color: "var(--t2)" }}>
            <Menu size={20} />
          </button>
          <span className="font-bold text-sm" style={{ color: "var(--t1)", fontFamily: "var(--font-display)" }}>Management Panel</span>
          <button onClick={logout} style={{ color: "var(--t3)" }}><LogOut size={18} /></button>
        </header>
